/*global Crafty:false, navigator:false*/
(function () {
  'use strict';
  ...
  Crafty.c('GamepadMultiway', {
    ...
    _calcSpeed: function (speed) {
      for (var b in this._BUTTONS_DIRECTION) {
        this._buttons[b] = {
          x: Math.round(Math.cos(this._BUTTONS_DIRECTION[b] * (Math.PI / 180)) * 1000 * speed.x) / 1000,
          y: Math.round(Math.sin(this._BUTTONS_DIRECTION[b] * (Math.PI / 180)) * 1000 * speed.y) / 1000
        };
      }
      ...
      for (var a in this._AXES_DIRECTION) {
        this._axes[a] = [];
        this._axes[a][0] = {
          x: Math.round(Math.cos(this._AXES_DIRECTION[a][0] * (Math.PI / 180)) * 1000 * speed.x) / 1000,
          y: Math.round(Math.sin(this._AXES_DIRECTION[a][0] * (Math.PI / 180)) * 1000 * speed.y) / 1000
        };
        this._axes[a][1] = {
          x: Math.round(Math.cos(this._AXES_DIRECTION[a][1] * (Math.PI / 180)) * 1000 * speed.x) / 1000,
          y: Math.round(Math.sin(this._AXES_DIRECTION[a][1] * (Math.PI / 180)) * 1000 * speed.y) / 1000
        };
      }
    },
