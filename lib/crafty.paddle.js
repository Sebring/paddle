	...
	events: {
		'GamepadAxisChange': function(e) {
			...
			if (this.pr_relative) {
				...
				// absolute
				this.rotation = this.pr_steps + this.pr_steps * -e.value
			}
		}
	}
})
...
Crafty.c('PaddleAxis', {
	...
	paddleAxis: function(id=0, axis=[0], resolution=1024) {
		this.paddleAxis = axis
		this.paddleRes = resolution
		this.gamepad(id)
		return this
	},
	events: {
		'GamepadAxisChange': function(e) {
			if(~this.paddleAxis.indexOf(e.axis)) {
				let value = (e.value * this.range) | 0
				if (e.value !== this.lastValue[e.axis])
				this.trigger('PaddleAxisChange', {axis:e.axis, value})
				this.lastValue[e.axis] = value
			}
		}
	}
})
...
Crafty.c('Thrust', {
	...
	thrust(id = 0, power = 100) {
		...
		this.paddleRotation(id, false, 360)
		return this
	},
	thrustGravity(gx, gy) {
		this.gravityX = gx
